...
app.post('/lighttimer', function(req, res) {
    ...
    // lightTimers.forEach is async. This for loop isn't.
    for (let i = 0; i < lightTimers.length; i++) {
        if (lightTimers[i].lightId === req.body.lightId) {
            log.warn('Timer already running for light ID ' + req.body.lightId);
            res.status(constants.HTTP_INVALID_ENTITY).send('Timer already running for light ID ' + req.body.lightId);
            return;
        }
    }

    let timer = setTimeout(() => {
        log.debug("Turning off light ID " + req.body.lightId + " now");
        clearTimeout(timer);
    ...
    lightTimers.push({ "lightId": req.body.lightId, "timer": timer });
    res.status(constants.HTTP_OKAY).send('OK');
});
...
// debug start
    setInterval(() => {
        lightTimers.forEach(item => {
            log.debug("Found light timer for light ID: " + item.lightId);
        });
    }, 10000);
// debug end
...
app.post('/toggle', function(request, response) {
    ...
    getLight(request.body.id, function(light) {
        getLightState(light, function(currentState) {
            ...
            log.debug('Light %s current state: %s', request.body.id, currentState);
...
function getLightState(light, callback) {
    ...
    request.get(util.format('http://%s:%d/ZWaveAPI/Run/devices[%d].instances[%d].commandClasses[0x25].data.level.value', light.controllerHost, light.controllerPort, light.device, light.instance), function(err, res, body) {
        ...
        if (res.statusCode === 200 && body.length > 0) {
            ...
            log.debug('Light %d is currently %s', light.id, simpleState);
...
function setLight(light, state) {
    ...
    log.debug('Setting light ID %d to %s', light.id, state);

    request.post(util.format('http://%s:%d/ZWaveAPI/Run/devices[%d].instances[%d].commandClasses[0x25].Set(%s)', light.controllerHost, light.controllerPort, light.device, light.instance, state), function(err, res, body) {
