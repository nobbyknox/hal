...
app.get('/scenes', function(req, res) {
    ...
    dataStore.get(models.SceneModel, where, {}, function(err, scenes) {
        if (err) {
            res.status(constants.HTTP_INVALID_ENTITY).send(err);
        } else {
...
app.get('/scenes/:id', (req, res) => {
    dataStore.get(models.SceneModel, { 'id': req.params.id }, {}, function(err, scenes) {
        if (err) {
            res.status(constants.HTTP_INVALID_ENTITY).send(err);
        } else {
            res.status(constants.HTTP_OKAY).send(JSON.stringify(scenes[0]));
        }
    });
});
