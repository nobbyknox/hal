...
let centerText = require('@nobbyknox/center-text');
...
let ON_VALUE = 'on';
let OFF_VALUE = 'off';
let razberryQueryPathTemplate = '/ZWaveAPI/Run/devices[%d].instances[%d].commandClasses[0x25].data.level.value';
let razberrySetPathTemplate = '/ZWaveAPI/Run/devices[%d].instances[%d].commandClasses[0x25].Set(%d)';
...
app.post('/combinedstatus', function(req, res) {

    let stats = [];

    log.debug('Number of running light timers: %d', lightTimers.length);

    lightService.get(1, (err, lights) => {
        ...
        if (err) {
            ...
            if (lights.length === 0) {
                res.status(constants.HTTP_STATUS.BAD_REQUEST).send('[]');
            } else {
                    ...
                    getLightState(light, function(err, currentState) {
                        if (err) {
                            ...
                            stats.push({
                                ...
                                name: light.name,
                                status: currentState,
                                onTimer: isOnTimer(light.id)
                            });

                            remaining--;

                            if (remaining === 0) {
                                res.status(constants.HTTP_STATUS.OKAY).send(JSON.stringify(stats));
                            }
                        }
                    });
                });
        ...
        if (err) {
            ...
            getLightState(light, function(err, currentState) {
                if (err) {
                    ...
                    if (newState === OFF_VALUE) {
                        cancelTimer(light.id);
                    }
                }
            });
        }
...
app.post('/lighttimer', function(req, res) {

    if (isOnTimer(req.body.lightId)) {
        log.warn('Timer already running for light');
        res.status(constants.HTTP_STATUS.BAD_REQUEST).send('Timer already running for light');
        return;
    }

    lightService.find(req.body.lightId, (err, light) => {
        if (err) {
            log.error(err);
            res.status(constants.HTTP_STATUS.BAD_REQUEST).send(err);
        } else {
            setLight(light, ON_VALUE);

            let timer = setTimeout(() => {
                log.info('Timer turning off light "%s"', light.name);
                setLight(light, OFF_VALUE);
                clearTimeout(timer);

                let index = -1;

                for (let i = 0; i < lightTimers.length; i++) {
                    if (lightTimers[i].lightId === req.body.lightId) {
                        index = i;
                        break;
                    }
                }
                ...
                if (index > -1) {
                    lightTimers.splice(index, 1);
                }

            }, req.body.delay * 1000);

            log.info('Timer set for light "%s" with delay of %d', light.name, req.body.delay);

            lightTimers.push({ lightId: light.id, lightName: light.name, timer: timer });
            res.status(constants.HTTP_STATUS.OKAY).send('{}');
        }
    });
});

app.post('/scenetimer', function(req, res) {

    log.debug('Top of scenetimer');
    log.debug('Request body: %j', req.body);
...
log.info('');
log.info(centerText('Version ' + constants.HAL_VERSION, 22));
log.info('');
...
function cancelTimer(lightId) {
    for (let i = 0; i < lightTimers.length; i++) {
        if (lightTimers[i].lightId === lightId) {
            log.info('Timer cancelled for light "%s"', lightTimers[i].lightName);
            clearTimeout(lightTimers[i].timer);
            return;
        }
    }
}

function isOnTimer(lightId) {
    for (let i = 0; i < lightTimers.length; i++) {
        if (lightTimers[i].lightId === lightId) {
            return true;
        }
