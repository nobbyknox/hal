...
let ON_VALUE = 'on';
let OFF_VALUE = 'off';
// var razberryQueryPathTemplate = '/ZWave.zway/Run/devices[%d].instances[%d].commandClasses[37].data.level.value';
// var razberrySetPathTemplate = '/ZWave.zway/Run/devices[%d].instances[%d].commandClasses[37].Set(%d)';
    ...
    lightService.find(request.body.id, (err, light) => {
        getLightState(light, function(err, currentState) {
            ...
            if (err) {
                ...
                res.status(constants.HTTP_OKAY).send(currentState);
            }
        });
    });
});
        ...
        if (err) {
            ...
            getLightState(light, function(err, currentState) {
				if (err) {
					...
					res.status(constants.HTTP_OKAY).send(newState);
...
function getLightState(light, next) {
    ...
    // let url = util.format('http://%s:%d' + razberryQueryPathTemplate, light.controllerHost, light.controllerPort, light.device, light.instance);
    let zPath = util.format(razberryQueryPathTemplate, light.device, light.instance);

    log.debug('Razberry query: %s', zPath);

    var options = {
        url: util.format('http://%s:%s%s', light.controllerHost, light.controllerPort, zPath),
    ...
    // var request = http.request(options, function(err, httpResponse, body) {
    //         log.debug('err: %j', err);
    //         log.debug('response: %j', httpResponse);
    ...
    //     log.error('Problem with request: %j' + e);
    // });

    // request.on('response', function(response) {
    //     log.debug('status code: %s', response.statusCode);
    //     // log.debug('response body: %j', response.body);
    //     // console.dir(response);
    // });

    // request.on('data', function(response) {
    //     log.debug('data: %j', response);
    // });
...
function setLight(light, state) {
    ...
    // let zPath = '/ZWaveAPI/Run/devices[' + light.device + '].instances[' + light.instance + '].commandClasses[37].Set(' + (state === ON_VALUE ? 255 : 0) + ')';
    let zPath = util.format(razberrySetPathTemplate, light.device, light.instance, (state === ON_VALUE ? 255 : 0));

    log.debug('Razberry command: %s', zPath);
