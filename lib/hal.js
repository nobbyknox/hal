...
app.put('/scenes', function(req, res) {
    dataStore.update(models.SceneModel, { 'id': req.body.id }, {}, req.body, (err, updatedScene) => {
        if (err) {
            ...
            res.status(constants.HTTP_OKAY).send(JSON.stringify(updatedScene));
        }
    });
});
...
app.delete('/scenes/:id', function(req, res) {
    sceneService.deleteScene(req.params.id, (err) => {
        if (err) {
            log.error(err);
            res.status(constants.HTTP_INVALID_ENTITY).send(utilities.convertErrorToResponse(err));
        } else {
            res.status(constants.HTTP_OKAY).send('');
        }
    });
...
app.get('/scenelights', function(req, res) {
    sceneService.getSceneLights(req.query.sceneId, (err, sceneLights) => {
        if (err) {
            ...
            res.status(constants.HTTP_OKAY).send(JSON.stringify(sceneLights));
        }
    });
});
...
app.get('/scenelights/:id', function(req, res) {
    sceneService.findSceneLight(req.params.id, (err, sceneLight) => {
        if (err) {
            ...
            res.status(constants.HTTP_OKAY).send(JSON.stringify(sceneLight));
        }
    });
});
...
app.post('/scenelights', function(req, res) {
    sceneService.saveSceneLight(req.body, (err, newObj) => {
        if (err) {
            ...
            res.status(constants.HTTP_OKAY).send(JSON.stringify(newObj));
        }
    });
});
...
app.put('/scenelights', function(req, res) {
    sceneService.updateSceneLight(req.body, (err, updatedObj) => {
        if (err) {
            ...
            res.status(constants.HTTP_OKAY).send(JSON.stringify(updatedObj));
        }
    });
});
                ...
                if (err) {
                    ...
                    lights.forEach(item => {
                        if (item.enabledInScene) {
                            ...
                            log.info('Light %s in scene %s is disabled and will not be triggered', item.name, scene.name);
                        }
                    });
                }
            });
