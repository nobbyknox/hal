...
app.get('/lights', function(req, res) {
    ...
    // TODO: Remove this query parameter and all that uses it.
    if (req.query.sceneId) {
        lightService.getLightsOfScene(req.query.sceneId, function(err, lights) {
            if (err) {
...
app.get('/scenelights/:id', function(req, res) {
    sceneService.getSceneLights(req.params.id, (err, sceneLights) => {
        if (err) {
            res.status(constants.HTTP_INVALID_ENTITY).send(err);
        } else {
