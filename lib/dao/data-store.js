...
let knexConfig = require('./knex-config.js');
let knex = require('knex')(knexConfig[(process.env.HAL_ENV || 'development')]);
...
function create(model, data, next) {
    ...
    model
        ...
        // We need to tell knex that, although we have an ID, it is not an update, but an insert.
        .save(null, { method: 'insert' })
        .then(function(createdModel) {
            log.trace(createdModel.toJSON());
            next(null, createdModel.toJSON());
        })
        .catch(function(err) {
            log.error(err);
            next(err);
...
function raw(query, bindings, next) {

    log.trace('Raw query: %s', query);
    log.trace('Raw query bindings: %j', bindings);
...
// TODO: Remove the 'destroy' parameter. We don't use it.
function remove(model, where, whereNot, destroy, next) {
    ...
    if (destroy) {
        ...
        update(model, where, whereNot, data, next);
    }
}
        ...
        .query(function(db) {
            ...
            if (options.join) {
                db.join(options.join.tableName, function() {
                    ...
                    this.on(options.join.on[0], options.join.on[1], options.join.on[2]);

                    if (options.join.andOn && options.join.andOn.length > 0) {
                        options.join.andOn.forEach(function(item) {
                            _this.andOn(item[0], item[1], item[2]);
                        });
                    }
                });
            }
...
function update(model, where, whereNot, data, next) {
    ...
    model
        ...
        .then(function(updatedModel) {
            log.trace(updatedModel.toJSON());
            next(null, updatedModel.toJSON());
        }, function(err) {
